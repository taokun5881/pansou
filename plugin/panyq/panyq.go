...
// 提到包级后只编译一次，匹配行为不变。
var (
	panyqRe1 = regexp.MustCompile(`<script src="(/_next/static/[^"]+\.js)"`)
	panyqRe2 = regexp.MustCompile(`["\']([a-f0-9]{40})["\']{1}`)
	panyqRe3 = regexp.MustCompile(`"sign":"([^"]+)"`)
	panyqRe4 = regexp.MustCompile(`"sha":"([a-f0-9]{64})"`)
	panyqRe5 = regexp.MustCompile(`"hash","([^"]+)"`)
	panyqRe6 = regexp.MustCompile(`(https?://[^\s"'<>]+|magnet:\?[^\s"'<>]+)`)
	panyqRe7 = regexp.MustCompile(`《([^》]+)》`)
	panyqRe8 = regexp.MustCompile(`【([^】]+)】`)
)

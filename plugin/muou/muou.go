...
func (p *MuouAsyncPlugin) searchAtBase(client *http.Client, keyword, baseURL string) ([]model.SearchResult, error) {
...
func (p *MuouAsyncPlugin) enhanceWithDetails(client *http.Client, results []model.SearchResult, baseURL string) []model.SearchResult {
...
func (p *MuouAsyncPlugin) fetchDetailLinksAndImages(client *http.Client, baseURL, itemID string) ([]model.Link, []string) {
	// 性能统计
	start := time.Now()

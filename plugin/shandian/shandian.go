...
func (p *ShandianAsyncPlugin) enhanceWithDetails(client *http.Client, results []model.SearchResult, baseURL string) []model.SearchResult {
	...
	for _, result := range results {
		...
		go func(r model.SearchResult) {
			...
			// 获取详情页链接
			detailLinks := p.fetchDetailLinks(client, baseURL, itemID)
			r.Links = detailLinks
...
func (p *ShandianAsyncPlugin) fetchDetailLinks(client *http.Client, baseURL, itemID string) []model.Link {
